...
namespace overlay
{
	void InitimGuiImpl() {
		...
		ImGui::NewFrame();
		ImGui::GetIO().MouseDrawCursor = OVERLAY;

		if (spoof_call(GetAsyncKeyState, VK_INSERT) & 1)
		{
			OVERLAY = !OVERLAY;
			ui::SetMenuState(OVERLAY);
		}
		HWND Rust_HWND = FindWindow(NULL, L"Rust");
		if (Rust_HWND != NULL)
		{
			ui::Render();
			ui::Threads();
	...
	void overlay::Render() {
		_HWND = FindWindow(NULL, XR(L"Rust"));
